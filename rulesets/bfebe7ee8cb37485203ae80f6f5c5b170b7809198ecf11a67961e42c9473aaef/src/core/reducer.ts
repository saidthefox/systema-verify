...
import { liveDefinitionFor, nextDefinitionVersion } from "./definitions"
import { beginFx, cascadeEntry, fxCoinCredit, fxCoinDebit, fxStatus, refundMarketVotes, refundOpenStakes, ruleProvisionalAct, takeFx } from "./effects"
import { instanceAdjacency, ladderVia } from "./graph"
import { PROTOCOL_V2_FROM_SEQ, protocolActivationDialError } from "./activation"

export const COLLISION_PETITION_V2 = "COLLISION_PETITION_V2"
import { houseAgentMintsUsed } from "./founding-fleet"
import {
  applyCommit, applyContest, applyContestRebase, applyReveal, evaluateChallengeScreen, evaluateRaidScreen,
  executeChallengeRuling, executeRaidRuling, expireStaleCoalitions, replaceFeasible,
  staleDefinitionHead, sweepScreensV2, validateCommit, validateContest, validateContestRebase, validateReveal, validateScreenVote,
} from "./ladder"
import { FORGE_PROOF_V3, applyIngotForged, applyIngotMelted, validateIngotForged, validateIngotMelted } from "./forge-receipts"
...
export function validate(state: CoreState, e: Pick<EventEnvelope, "kind" | "v" | "actor" | "payload" | "ts">): string | null {
  ...
  switch (e.kind) {
    ...
    case "DIAL_SET": {
      ...
      const key = str(p, "key")
      if (!key || p.value === undefined) return "DIAL_SET needs key and value"
      if (key === PROTOCOL_V2_FROM_SEQ) return protocolActivationDialError(state, e.actor, p.value)
      if (key === COLLISION_PETITION_V2) {
        if (p.value !== true) return `${COLLISION_PETITION_V2} is a one-way activation and must be true`
        if (dialBool(state.dials, COLLISION_PETITION_V2)) return `${COLLISION_PETITION_V2} is already active`
      }
      if (key === FORGE_PROOF_V3) {
        if (p.value !== true) return `${FORGE_PROOF_V3} is a one-way activation and must be true`
        if (dialBool(state.dials, FORGE_PROOF_V3)) return `${FORGE_PROOF_V3} is already active`
      }
      return null
    }
    case "ALIAS_REVOKED": {
    ...
    case "AGENT_MINTED": {
      ...
      const house = state.houses[houseId]
      if (!house) return "no such house"
      if (houseAgentMintsUsed(house) >= dialNum(state.dials, "HOUSE_AGENT_SLOTS")) return "the mint grant is spent — one agent mint per house (Law 38)"
      return null
    }
    ...
    case "CONTEST_FILED":
      if (coupledCollisionEdge(state, str(p, "targetId"))) return "a declared collision has one judgment target — contest the filing, not its derived edge (Law 39)"
      return validateContest(state, e as EventEnvelope)
    case "CONTEST_REBASED":
      return validateContestRebase(state, e as EventEnvelope)
    case "VOTE_COMMIT":
    ...
    case "STAKE_PLACED": {
      ...
      if (tt !== "ENTRY" && tt !== "DEFINITION" && tt !== "EDGE" && tt !== "LABEL") return "attest targets entries, definitions, edges, and filed labels — a facet is a lens, not an act"
      ...
      if (act.collisionParentId) return "a declared collision has no independent attestation market — back the filing whose verdict it follows (Law 39)"
      if (act.kind === "LABEL" && act.canonical) return "the canonical name rides with its ENTRY — attest the entry; only separately filed labels have their own market"
    ...
    case "FLAG_FILED": {
      ...
      if (act.collisionParentId) return "a declared collision has one coherence target — flag the filing, not its derived edge (Law 39)"
    ...
    case "SMELT": {
      ...
      if (dialBool(state.dials, FORGE_PROOF_V3)) {
        const claimTo = str(p, "claimTo")
        if (!claimTo || !/^0x[0-9a-f]{40}$/.test(claimTo) || /^0x0{40}$/.test(claimTo)) {
          return "proof-gated smelting requires a lowercase nonzero EVM claimTo address fixed before the coins burn"
        }
      }
    ...
    case "INGOT_FORGED":
      return validateIngotForged(state, e)
    case "INGOT_MELTED":
      return validateIngotMelted(state, e)
    case "COURT_RULING": {
      ...
      if (tt === "CHALLENGE") {
        ...
        if (ruling === "ACCEPTED") {
          const stale = staleDefinitionHead(state, c)
          if (stale) return `challenge is stale against definition ${stale.id} — rebase and rejudge before upholding it`
        }
      ...
      if (act.collisionParentId) return "a declared collision follows its filing's verdict — the court does not rule its derived edge separately (Law 39)"
      ...
      if (act.collisionWithEntryId) {
        const proposedEntryId = act.kind === "ENTRY" ? act.id : null
...
function validateFiling(state: CoreState, e: EventEnvelope): string | null {
  ...
  if (kind === "ENTRY") {
    ...
    if (big) return big
    const collisionV2 = dialBool(state.dials, COLLISION_PETITION_V2)
    if (collisionV2 && !!str(p, "sense") !== !!str(p, "distinction")) return "provide collision sense AND distinction together — neither is an ordinary entry field (Law 39)"
    ...
    const held = nameHolders(state.names, norm(name))
    if (held.length) {
      if (!collisionV2) {
      ...
      // Law 39 / 13-i — the sense is no longer a password through the duplicate gate. It opens
      // a structured collision PETITION against the first holder: proposed sense plus a
      // concrete distinction the judges can test side-by-side. No semantic oracle is hidden
      // here; the court still decides whether this is one construct twice or a genuine split.
      const holder = state.acts[held[0]]
      const holderEntry = holder.kind === "ENTRY" ? holder.id : holder.entryId!
      if (!str(p, "sense") || !str(p, "distinction")) {
        return `name already held (Law 12/39) by ${holderEntry}: ${name} — a sense alone does not open the door; inspect the holder and petition the collision court with sense AND distinction`
      }
      const bigCollision = firstTooLong([
        ["sense", p.sense, LIMITS.COLLISION_SENSE],
        ["distinction", p.distinction, LIMITS.COLLISION_DISTINCTION],
      ])
      if (bigCollision) return bigCollision
    } else if (collisionV2 && (str(p, "sense") || str(p, "distinction"))) {
      return "collision sense and distinction belong only to a held word — this name has no existing holder to distinguish (Law 39)"
    }
    return null
  }
  if (kind === "DEFINITION") {
  ...
  if (kind === "LABEL") {
    ...
    const collisionV2 = dialBool(state.dials, COLLISION_PETITION_V2)
    if (collisionV2 && !!str(p, "sense") !== !!str(p, "distinction")) return "provide collision sense AND distinction together — neither is an ordinary label field (Law 39)"
    ...
    if (others.length) {
      ...
      const holder = state.acts[others[0]]
      const holderEntry = holder.kind === "ENTRY" ? holder.id : holder.entryId!
      if (!str(p, "sense") || !str(p, "distinction")) {
        return `word already held (Law 39) by ${holderEntry}: ${text} — a sense alone does not open the door; inspect the holder and petition the collision court with sense AND distinction`
      }
      const bigCollision = firstTooLong([
        ["sense", p.sense, LIMITS.COLLISION_SENSE],
        ["distinction", p.distinction, LIMITS.COLLISION_DISTINCTION],
      ...
      return "collision sense and distinction belong only to a word held by another concept (Law 39)"
    }
    return null
  }
...
function applyAcceptedV1(state: CoreState, e: EventEnvelope, notes: string[]): Outcome {
  ...
  switch (e.kind) {
    ...
    case "COMPENSATION": {
      ...
      fxCoinCredit(rec.fp, rec.amountBase, "COMPENSATION", `burn:${str(p, "burnSeq")}`)
    ...
    case "CONTEST_FILED":
      ...
      break
    case "CONTEST_REBASED":
      applyContestRebase(state, e, notes)
      break
    case "VOTE_COMMIT":
    ...
    case "SMELT": {
      ...
      fxCoinDebit(entityFp, whole * COIN, "SMELT", `ingot:${e.seq}`)
    ...
    case "INGOT_FORGED":
      applyIngotForged(state, p)
      notes.push(`ingot ${str(p, "ingotId")} forged as token ${str(p, "tokenId")} on chain ${str(p, "chainId")}`)
      break
    case "INGOT_MELTED":
      applyIngotMelted(state, p)
      notes.push(`ingot ${str(p, "ingotId")} melted; its fungible pour is externally receipted`)
      break
    case "COURT_RULING": {
      const tt = str(p, "targetType")
      const tid = str(p, "targetId")!
...
function applyFiling(state: CoreState, e: EventEnvelope, notes: string[]) {
  ...
  if (kind === "ENTRY") {
    ...
    if (holders.length) {
      ...
      // already demanded the sense and distinction. Both kingdoms author this edge as
      // collision:<actId>.
      const holder = state.acts[holders[0]]
      const holderEntry = holder.kind === "ENTRY" ? holder.id : holder.entryId!
      if (collisionV2) {
        act.collisionWithEntryId = holderEntry
        act.collisionSense = str(p, "sense")!
        act.collisionDistinction = str(p, "distinction")!
      }
      deriveCollisionEdge(state, notes, e.actor, e.seq, actId, actId, holderEntry, collisionV2)
    }
    ...
    if (other) {
      ...
      deriveCollisionEdge(state, notes, e.actor, e.seq, actId, act.entryId!, holderEntry, collisionV2)
    }
  }
...
function deriveCollisionEdge(
  ...
  filingActId: string, fromEntryId: string, toEntryId: string, coupled = false,
) {
  ...
  state.acts[edgeId] = {
    ...
    ...(coupled ? { collisionParentId: filingActId } : {}),
  }
  notes.push(coupled
    ? `collision edge derived ${edgeId} (Law 39): coupled to ${filingActId}; distinct concepts sharing a word — or one construct twice?`
    : `collision edge derived ${edgeId} (Law 39): distinct concepts sharing a word — or one construct twice?`)
}

function coupledCollisionEdge(state: CoreState, targetId: string | null): boolean {
  return !!(targetId && state.acts[targetId]?.collisionParentId)
}
...
function applyStake(state: CoreState, e: EventEnvelope, notes: string[]) {
  ...
  fxCoinDebit(e.actor, amount, "STAKE", key)
  ...
  if (act.holdback && !act.holdback.released && amount >= dialBig(state.dials, "HOLDBACK_RELEASE_MIN_BASE")) {
    ...
    fxCoinCredit(act.authorFp, act.holdback.heldBase, "HOLDBACK_RELEASE", `${act.kind}:${act.id}`)
