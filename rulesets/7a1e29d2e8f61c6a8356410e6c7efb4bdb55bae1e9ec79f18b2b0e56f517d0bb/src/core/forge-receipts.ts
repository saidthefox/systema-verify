...
export function applyIngotForged(state: CoreState, payload: Record<string, unknown>) {
  ...
  const { ingotId: _ignored, ...receipt } = payload as unknown as IngotForgeReceipt & { ingotId: string }
  ingot.forge = receipt
}

export function applyIngotMelted(state: CoreState, payload: Record<string, unknown>) {
  const ingot = state.ingots[text(payload, "ingotId")!]
  const { ingotId: _ignored, ...receipt } = payload as unknown as IngotMeltReceipt & { ingotId: string }
  ingot.melt = receipt
}
