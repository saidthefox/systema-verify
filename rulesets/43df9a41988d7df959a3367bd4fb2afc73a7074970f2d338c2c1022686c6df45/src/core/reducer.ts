...
function validateFiling(state: CoreState, e: EventEnvelope): string | null {
  ...
  if (kind === "ENTRY") {
    ...
    if (big) return big
    if (!!str(p, "sense") !== !!str(p, "distinction")) return "provide collision sense AND distinction together — neither is an ordinary entry field (Law 39)"
    ...
    } else if (str(p, "sense") || str(p, "distinction")) {
  ...
  if (kind === "LABEL") {
    ...
    if (!!str(p, "sense") !== !!str(p, "distinction")) return "provide collision sense AND distinction together — neither is an ordinary label field (Law 39)"
...
function deriveCollisionEdge(
  ...
  state.acts[edgeId] = {
    ...
    collisionParentId: filingActId,
  }
  notes.push(`collision edge derived ${edgeId} (Law 39): coupled to ${filingActId}; distinct concepts sharing a word — or one construct twice?`)
}
