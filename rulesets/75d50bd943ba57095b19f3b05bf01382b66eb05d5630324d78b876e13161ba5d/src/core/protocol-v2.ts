...
const COMMAND_TO_FACT = {
  ...
  Smelt: "SMELT",
  RecordForgeReceipt: "INGOT_FORGED",
  RecordMeltReceipt: "INGOT_MELTED",
  RuleCourt: "COURT_RULING",
} as const

export type CommandKindV2 = keyof typeof COMMAND_TO_FACT

const FACT_TO_COMMAND = Object.fromEntries(
  Object.entries(COMMAND_TO_FACT).map(([command, fact]) => [fact, command]),
) as Record<string, CommandKindV2>

/** Route-era fact vocabulary to v2 imperative intent. Derived-only facts intentionally return null. */
export function commandForFactV2(factKind: string): CommandKindV2 | null {
  return FACT_TO_COMMAND[factKind] ?? null
}
...
export function decideV2(state: CoreState, command: CommandEnvelopeV2, ts: string): DecisionV2 {
  ...
  let reason = validate(view, { ...fact, ts })
  // Keeper's economy ruling, 2026-09-02 — ordinary definition filings fill a blank; they do
  // not create an endlessly repeatable four-coin revision lane. Keep this at the v2 command
  // boundary rather than in v1 fact validation: historical v1 records legitimately contain
  // successive versions and must remain constitutionally replayable. Rejected/superseded
  // carvings are dead and free the lane; an upheld Law 30 REPLACE seats its successor through
  // the ruling path, not FileAct, so corrections remain fully rewarded.
  if (!reason && command.command === "FileAct" && command.payload.actKind === "DEFINITION") {
    const entryId = typeof command.payload.entryId === "string" ? command.payload.entryId : ""
    const live = entryId ? liveDefinitionFor(view, entryId) : null
    if (live) {
      reason = live.status === "ACCEPTED"
        ? `entry already has confirmed definition ${live.id} — correct it by contesting that definition with mode REPLACE (Law 30)`
        : `entry already has provisional definition ${live.id} — judge that carving before filing another`
    }
  }
